...
fn eval<'py>(
    ...
    if has_contiguous_rows(&coeffs) && has_contiguous_rows(&values) {
        ...
        for (coeffs, values) in iter::zip(coeffs.rows(), values.rows()) {
            result.push(
                nutils_poly::eval(coeffs, &values, degree)
                    .map_err(|err| PyValueError::new_err(err.to_string()))?,
            );
        }
    }
...
fn eval_outer<'py>(
    ...
    if has_contiguous_rows(&coeffs) && has_contiguous_rows(&values) {
        ...
        for values in values.rows() {
            contig_values.assign(values.copied().iter());
            for coeffs in coeffs.rows() {
                result.push(
                    nutils_poly::eval(coeffs.as_slice().unwrap(), &contig_values, degree)
                        .map_err(|err| PyValueError::new_err(err.to_string()))?,
                );
            }
        }
        ...
        for values in values.rows() {
            ...
            for coeffs in coeffs.rows() {
                result.push(
                    nutils_poly::eval(coeffs, &contig_values, degree)
                        .map_err(|err| PyValueError::new_err(err.to_string()))?,
                );
            }
        }
